...
pub mod get {
    ...
    pub fn get_started_entrypoints() -> Result<Vec<String>, CommandError> {
        ...
        let started_path =
            batbelt::path::get_folder_path(batbelt::path::BatFolder::CodeOverhaulStarted, true)
                .change_context(CommandError)?;
        ...
        if started_files.is_empty() {
            return Err(Report::new(CommandError)
                .attach_printable("no started files in code-overhaul folder"));
        }
        Ok(started_files)
    }

    // pub fn get_finished_co_files() -> Result<Vec<(String, String)>, CommandError> {
    ...
    pub fn get_only_files_from_folder(folder_path: String) -> Result<Vec<FileInfo>, CommandError> {
...
pub mod count {
    ...
    pub fn co_counter() -> error_stack::Result<(usize, usize, usize), CommandError> {
        ...
        let to_review_path =
            batbelt::path::get_folder_path(batbelt::path::BatFolder::CodeOverhaulToReview, true)
                .change_context(CommandError)?;
        ...
        let finished_path =
            batbelt::path::get_folder_path(batbelt::path::BatFolder::CodeOverhaulFinished, true)
                .change_context(CommandError)?;
...
// pub fn read_to_string(path: &str) -> error_stack::Result<String, CommandError> {}
pub fn fs_read_dir(path: &str) -> error_stack::Result<ReadDir, CommandError> {
    let dir = fs::read_dir(path)
        .into_report()
        .change_context(CommandError)
        .attach_printable_lazy(|| format!("Error reading dir: \n path: {} ", path))?;
    // let dir = fs::read_dir(path).map_err(|_| CommandErrorType::ReadDir { path }.parse_error())?;
    Ok(dir)
}

// pub fn fs_write(path: &str, content: &str) -> error_stack::Result<(), CommandError> {
//     fs::write(path, content).map_err(|_| CommandErrorType::Write { path }.parse_error())?;
//     Ok(())
// }
