...
pub mod helpers {
    ...
    pub mod parse {
        ...
        pub fn validations_into_co(
            ...
            let mut validations: Vec<String> = super::get::possible_validations(
            ...
            for (line_number, line) in filtered_lines.iter().enumerate() {
                if line.contains("#[account(") {
                    ...
                    if !(account_string[1].contains("#[account(")
                        && account_string[2].contains(")]"))
        ...
        pub fn function_parameters_into_co(co_file_path: String, co_file_name: String) {
            ...
            let entrypoint_text =
                "pub pub fn ".to_string() + co_file_name.replace(".md", "").as_str();
            ...
            match first_line_index_opt {
                ...
                .unwrap();
                    ...
                    let instruction_file =
                        File::open(selected_instruction_file.path.clone()).unwrap();
                    let instruction_file_lines = io::BufReader::new(instruction_file)
                        .lines()
                        .map(|l| l.unwrap())
                        .collect::<Vec<String>>();
                    ...
                    && line.contains('(')
                {
                    ...
                    if line.contains(");") || line.contains(")?;") {
                        ...
                        let mut validation_line =
                            instruction_file.clone().lines().collect::<Vec<_>>()[line_index + idx]
                                .to_string();
...
fn get_instruction_file_with_prompts(to_start_file_name: &String) -> (String, String) {
    let instruction_files_info = helpers::get::instruction_files();
...
pub fn co_counter() -> (usize, usize, usize) {
    ...
    let to_review_count = helpers::count_filtered(to_review_folder);
    let started_path = BatConfig::get_auditor_code_overhaul_started_path(None);
    let started_folder = fs::read_dir(started_path).unwrap();
    let started_count = helpers::count_filtered(started_folder);
    let finished_path = BatConfig::get_auditor_code_overhaul_finished_path(None);
    let finished_folder = fs::read_dir(finished_path).unwrap();
    let finished_count = helpers::count_filtered(finished_folder);
